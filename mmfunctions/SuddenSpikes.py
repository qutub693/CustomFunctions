...
class VerySimpleModel:
    def __init__(self, input_item):
        if self.input_item>6000:
            self.Anomaly=input_item
        else:
            self.Anomaly=""

class SuddenSpikes(SupervisedLearningTransformer):
...
#         raise Exception(log_stuff)
        ...
        if very_simple_model is None and self.auto_train:
            ...
            very_simple_model = VerySimpleModel(input_item)
